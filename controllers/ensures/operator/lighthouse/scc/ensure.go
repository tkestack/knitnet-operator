...
func Ensure(restConfig *rest.Config, namespace string) (bool, error) {
	agentSaName, err := embeddedyamls.GetObjectName(embeddedyamls.Manifests_config_rbac_lighthouse_agent_service_account_yaml)
	if err != nil {
		return false, err
	}

	coreDNSSaName, err := embeddedyamls.GetObjectName(embeddedyamls.Manifests_config_rbac_lighthouse_coredns_service_account_yaml)
	if err != nil {
		return false, err
	}
